...
VajroSDK.subscribe(VajroSDK.Triggers.LINE_ITEM_ADDED_TO_CART, (appContext, lineItem) => {
	// output.innerHTML = output.innerHTML + `<div>LINE_ITEM_ADDED_TO_CART trigger invoked</div>`;
	let { productId, quantity } = lineItem
	let {
		cartLineItems: { totalAfterSavings, lineItems }
	} = appContext
	let extraproduct
...
VajroSDK.subscribe(VajroSDK.Triggers.LINE_ITEM_UPDATED, (appContext, updateType, lineItem) => {
	...
	if (updateType === "Increment") {
		...
		if (totalAfterSavings > 3000) {
			...
			if (extraproduct3thous != "7041966440639") {
				VajroSDK.removeLineItemFromCart(aboveLineItemHandle1000, 0)
					.then(() => {})
					.catch(err => {})
				addThreeThousAboveProduct()
			}
			...
			if (extraproduct != "6733310853311") {
				VajroSDK.removeLineItemFromCart(aboveLineItemHandle3000, 0)
					.then(() => {})
					.catch(err => {})
				addThousAboveProduct()
			}
		...
		if (productId === "6928830267583") {
			...
			if (subLineItemHandle) {
				VajroSDK.removeLineItemFromCart(subLineItemHandle, quantity)
					.then(() => {})
					.catch(err => {})
			}
		} else {
		...
		if (totalAfterSavings < 3000 && extraproduct3thous == "7041966440639") {
			...
			for (let { lineItemHandle, productId: id } of lineItems) {
				if (id === "7041966440639") {
					// subLineItemHandle = lineItemHandle;
					// break;
					VajroSDK.removeLineItemFromCart(lineItemHandle, 0)
						.then(() => {})
						.catch(err => {})
					addThousAboveProduct()
					VajroSDK.removeLineItemFromCart(aboveLineItemHandle3000, 0)
			...
			if (subLineItemHandle) {
				VajroSDK.removeLineItemFromCart(subLineItemHandle, 0)
					.then(() => {})
					.catch(err => {})
				progressbarcalc(totalAfterSavings, 1000)
				VajroSDK.removeLineItemFromCart(aboveLineItemHandle1000, 0)
...
function progressbarcalc(totalAfterSavings, aboveamount) {
	...
	const percentageamount =
		(totalAfterSavings / aboveamount) * 100 > 100
			? 100
			: (totalAfterSavings / aboveamount) * 100
	gsap.to(progressBar, {
		x: `${percentageamount}%`,
		duration: 2,
